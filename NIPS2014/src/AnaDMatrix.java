...
public class AnaDMatrix {
	...
	public void anaLogs()
	{
		...
		for(int subID=10;subID<21;subID++)
		{
			File folder = new File("./logs/"+subID);
			...
			for (int i = 0; i < listOfFiles.length; i++) {
				if (listOfFiles[i].isFile()) {
					...
					if (files.startsWith(fileFilter)) {
						...
						List<String> currentFileContent = DicccolUtilIO.loadFileToArrayList("./logs/"+subID+"/" + files);
	...
	public void calSingleSubSingleRunCorre()
	{
		...
		for(int subID=1;subID<11;subID++)
		{
			for(int round=0;round<10;round++)
			{
				...
				String currentFile = "./DMatrix/"+subID+".MOTOR_Dmat_dsize_400_lamda_8non_r_"+round+"_.txt";
				System.out.println("Dealling with "+currentFile);
				double[][] currentSigM = this.trsposeM(DicccolUtilIO.loadFileAsArray(currentFile, tSize, dicSize), tSize, dicSize);
				for(int i=0;i<dicSize;i++)
					for(int j=0;j<dicSize;j++)
						currentCorreM[i][j] = correlation.Correlation_Pearsons(currentSigM[i], currentSigM[j]);
				DicccolUtilIO.writeArrayToFile(currentCorreM, dicSize, dicSize, " ", "./DMatrixCorre/sub_"+subID+"_r_"+round+"_corre.txt");
				DicccolUtilIO.writeVtkMatrix1(currentCorreM, dicSize, dicSize, "./DMatrixCorre/sub_"+subID+"_r_"+round+"_corre.vtk");				
			} //for all rounds
		} // for every subject
	}
	...
	public List<Integer> updateSigIndexList(List<Integer> currentList, double[][] seedSigM, double[][] currentSigM, Correlation correlation, int dicSize)
	{
		...
		for(int i=0;i<currentList.size();i++)
		{
			...
			{
				double tmpCorreValue = correlation.Correlation_Pearsons(seedSigM[tmpSigIndex], currentSigM[d]);
...
//					System.out.println("Preserve "+tmpSigIndex+"...");
	...
	public List<Integer> finalizeStableSigIndex (List<Integer> currentIndexList, double[][] data, Correlation correlation, int dicSize)
	{
		...
		do
		{
			...
			{
				double tmpCorreValue = correlation.Correlation_Pearsons(data[currentIndexList.get(0)], data[currentIndexList.get(i)]);
				if(tmpCorreValue>=correLationThreshold)
				{
...
//					System.out.println("Loading " + currentFile);
			...
			System.out.println("Total "+stableSigIndexList.size()+": "+stableSigIndexList);
		...
		for (int subID = 1; subID < 11; subID++) 
		{
			...
			double[][] currentSigM = this.trsposeM(
					...
					tSize, dicSize);
			System.out.println("For sub "+subID+", ther are original "+allStableSigIndexList.get(subID-1).size()+" stable signals...");
			List<Integer> currentFinalizeList = this.finalizeStableSigIndex(allStableSigIndexList.get(subID-1), currentSigM, correlation, dicSize);
			System.out.println("Now, there are "+currentFinalizeList.size()+" template signals...");
			...
			DicccolUtilIO.writeArrayToFile(finalSigTemplate, currentFinalizeList.size(), tSize, " ", "./DMatrixTemplate/" + subID+ ".MOTOR_Dmat_Template.txt");
			
			double[][] currentCorreM = new double[currentFinalizeList.size()][currentFinalizeList.size()];
			for(int i=0;i<currentFinalizeList.size();i++)
				for(int j=0;j<currentFinalizeList.size();j++)
					currentCorreM[i][j] = correlation.Correlation_Pearsons(finalSigTemplate[i], finalSigTemplate[j]);
			DicccolUtilIO.writeArrayToFile(currentCorreM, currentFinalizeList.size(), currentFinalizeList.size(), " ", "./DMatrixTemplate/sub_"+subID+".MOTOR_Template_corre.txt");
			DicccolUtilIO.writeVtkMatrix1(currentCorreM, currentFinalizeList.size(), currentFinalizeList.size(), "./DMatrixTemplate/sub_"+subID+".MOTOR_Template_corre.vtk");				
		} //for every subject
	...
	public void test()
	{
		double[][] currentSigM = this.trsposeM(DicccolUtilIO.loadFileAsArray("test.txt", 3, 5), 3, 5);
